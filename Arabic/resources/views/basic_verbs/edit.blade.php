...
<div class="container">
    <div class="content-card">
        <div class="card-header">
            <h2 class="card-title">تعديل الفعل الثلاثي</h2>
            <a href="{{ route('basic-trilateral-verbs.index') }}" class="btn btn-sm">العودة للقائمة</a>
        </div>

        <form action="{{ route('basic-trilateral-verbs.update', $basicTrilateralVerb) }}" method="POST">
            @csrf
            @method('PUT')
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1rem;">
                <div style="margin-bottom: 1rem;">
                    ...
                    <input type="text" name="pattern" id="pattern" value="{{ old('pattern', $basicTrilateralVerb->pattern) }}" required 
                ...
                <div style="margin-bottom: 1rem;">
                    ...
                    <input type="text" name="past_tense" id="past_tense" value="{{ old('past_tense', $basicTrilateralVerb->past_tense) }}" required
                        style="width: 100%; padding: 0.75rem; border: 1px solid rgba(0,0,0,0.2); border-radius: 6px; font-family: 'Amiri', serif;">
                    @error('past_tense')
                        <span style="color: #dc3545; font-size: 0.9rem; display: block; margin-top: 0.5rem;">{{ $message }}</span>
                    @enderror
                ...
                <div style="margin-bottom: 1rem;">
                    ...
                    <input type="text" name="present_tense" id="present_tense" value="{{ old('present_tense', $basicTrilateralVerb->present_tense) }}" required
                        style="width: 100%; padding: 0.75rem; border: 1px solid rgba(0,0,0,0.2); border-radius: 6px; font-family: 'Amiri', serif;">
                    @error('present_tense')
                        <span style="color: #dc3545; font-size: 0.9rem; display: block; margin-top: 0.5rem;">{{ $message }}</span>
                    @enderror
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <textarea name="syntactic_analysis" id="syntactic_analysis" rows="3"
                    style="width: 100%; padding: 0.75rem; border: 1px solid rgba(0,0,0,0.2); border-radius: 6px; font-family: 'Amiri', serif;">{{ old('syntactic_analysis', $basicTrilateralVerb->syntactic_analysis) }}</textarea>
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <textarea name="example_sentence" id="example_sentence" rows="3"
                    style="width: 100%; padding: 0.75rem; border: 1px solid rgba(0,0,0,0.2); border-radius: 6px; font-family: 'Amiri', serif;">{{ old('example_sentence', $basicTrilateralVerb->example_sentence) }}</textarea>
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <textarea name="notes" id="notes" rows="3"
                    style="width: 100%; padding: 0.75rem; border: 1px solid rgba(0,0,0,0.2); border-radius: 6px; font-family: 'Amiri', serif;">{{ old('notes', $basicTrilateralVerb->notes) }}</textarea>
                @error('notes')
                    <span style="color: #dc3545; font-size: 0.9rem; display: block; margin-top: 0.5rem;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="text-align: center; margin-top: 1.5rem;">
                <button type="submit" class="btn" style="min-width: 200px;">تحديث</button>
            </div>
        </form>
    </div>
</div>
